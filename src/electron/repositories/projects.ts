...
import {projects, type ProjectSelect} from '../db/schema/projects.ts'
import  { db } from '../db/index.ts'
import type {ProjectInsert} from '../db/schema/projects.ts'
import type {CreateProjectData, UpdateProjectData} from "../shared/schemas/projects.ts"


export class ProjectRepo {
    async create(data: CreateProjectData): Promise<ProjectInsert> {
    ...
    async findById(project_id: string): Promise<ProjectSelect | null> {
        const result = await db
            .select()
            .from(projects)
